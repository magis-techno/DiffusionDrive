...
class TrajectoryDataManager:
    ...
    def load_frame_data(self, scene_token: str, frame_idx: int) -> Dict[str, Any]:
        """
        Load data for a specific frame in a scene
        
        Args:
            scene_token: Scene token
            frame_idx: Index of the frame to load (0-based)
        ...
        logger.debug(f"Loading frame {frame_idx} data for scene: {scene_token}")
        
        # Load scene
        scene = self.scene_loader.get_scene_from_token(scene_token)
        
        # Validate frame index
        if frame_idx < 0 or frame_idx >= len(scene.frames):
            raise ValueError(
                f"Frame index {frame_idx} out of range. "
                f"Scene has {len(scene.frames)} frames (0-{len(scene.frames)-1})"
            )
        
        # Get specific frame
        target_frame = scene.frames[frame_idx]
        
        # Extract sensor data from this frame
        sensors = {
            "cameras": target_frame.cameras,
            "lidar": target_frame.lidar
        }
        
        # Extract map and ego information from this frame
        map_info = {
            "api": scene.map_api,
            "ego_pose": target_frame.ego_status.ego_pose,
            "ego_status": target_frame.ego_status
        }
    ...
    def get_trajectories_from_frame(self, scene_token: str, frame_idx: int, horizon: float = 3.0) -> Dict[str, Any]:
        """
        Get trajectories starting from a specific frame
        ...
        Returns:
            Dictionary containing trajectories from the specified frame
        """
        logger.debug(f"Getting trajectories from frame {frame_idx} for scene: {scene_token}")
        
        # Load the scene to get frame timing information
        scene = self.scene_loader.get_scene_from_token(scene_token)
        
        if frame_idx >= len(scene.frames):
            raise ValueError(f"Frame index {frame_idx} out of range for scene with {len(scene.frames)} frames")
        
        # Get ground truth trajectory from this frame onwards
        gt_trajectory = self._extract_gt_trajectory_from_frame(scene, frame_idx, horizon)
    ...
    def _extract_gt_trajectory_from_frame(self, scene, frame_idx: int, horizon: float):
        """Extract GT trajectory starting from a specific frame"""
        # Calculate how many future frames we need based on horizon
        # Assuming 10Hz frame rate (0.1s per frame)
        future_frames_needed = int(horizon / 0.1)
        
        poses = []
        start_frame = frame_idx
        
        for i in range(future_frames_needed + 1):  # +1 to include current frame
            target_frame_idx = start_frame + i
    ...
    def _extract_pdm_trajectory_from_frame(self, cache_entry, frame_idx: int, horizon: float):
        """Extract PDM trajectory segment starting from a specific frame"""
        try:
            # This is a simplified extraction - in reality you might need to 
            # align the PDM trajectory timing with the specific frame
            trajectory = cache_entry.trajectory
            if hasattr(trajectory, 'poses') and len(trajectory.poses) > frame_idx:
                # Calculate how many points we need for the horizon
                points_needed = int(horizon / 0.1)  # Assuming 10Hz
                
                start_idx = frame_idx
                end_idx = min(start_idx + points_needed + 1, len(trajectory.poses))
            ...
            logger.warning(f"Failed to extract PDM trajectory from frame {frame_idx}: {e}")
            return None
